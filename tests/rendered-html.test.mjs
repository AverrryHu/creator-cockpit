...
test("keeps device-local storage and AI fallback contracts in source", async () => {
  ...
  assert.match(page, /模板不会消失，可以重复拖入日历/);
  assert.match(page, /拖动创建复盘日/);
  assert.match(page, /拖动创建直播日程/);
  assert.match(page, /function LiveSessionModal/);
  assert.match(page, /直播内容 \/ 流程/);
  assert.match(page, /复盘日、直播日可重复创建/);
  ...
  assert.match(storage, /schemaVersion:\s*10/);
