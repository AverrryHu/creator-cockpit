...
test("renders the creator cockpit shell", async () => {
  ...
  assert.match(html, /灵感池/);
  assert.match(html, /档期规划/);
  assert.match(html, /今天要完成的阶段/);
  assert.match(html, /大目标/);
  assert.doesNotMatch(html, /大目标（阶段）/);
...
test("keeps device-local storage and AI fallback contracts in source", async () => {
  ...
  assert.match(page, /\{ id: "schedule", label: "档期规划", icon: "schedule" \}/);
  ...
  assert.match(page, /\{ id: "inspirations", label: "灵感池", icon: "inspiration" \}/);
  assert.match(page, /\{ id: "pipeline", label: "内容总览", icon: "pipeline" \}/);
  assert.match(page, /\{ id: "goals", label: "大目标", icon: "goals" \}/);
  assert.doesNotMatch(page, />搜索内容<\/button>/);
  assert.match(page, /function InspirationPoolView/);
  assert.match(page, /function InspirationDetailModal/);
  assert.match(page, /className="inspiration-card-open"/);
  assert.match(page, /保存修改/);
  assert.match(page, /灵感只是等待判断的想法，不计入内容、目标或复盘/);
  assert.match(page, /function CreateContentModal/);
  assert.match(page, /从灵感池选择/);
  assert.match(page, /function ContentOverviewView/);
  assert.match(page, /流程<\/span><small>Pipeline/);
  assert.match(page, /列表<\/span><small>List/);
  assert.match(page, /全部阶段/);
  assert.match(page, /全部档位/);
  assert.match(page, /全部优先级/);
  ...
  assert.match(styles, /\.mobile-nav\s*\{[^}]*grid-template-columns:\s*repeat\(6,\s*1fr\)/);
  assert.match(page, /sidebarCollapsed/);
  assert.doesNotMatch(page, /className="theme-toggle"/);
  assert.match(page, /className="ghost-button topbar-theme-toggle"/);
  assert.match(page, /深色模式/);
  assert.match(page, /creator-cockpit-theme/);
  assert.match(page, /creator-cockpit-style/);
  assert.match(layout, /prefers-color-scheme: dark/);
  assert.match(layout, /document\.documentElement\.dataset\.theme/);
  assert.match(layout, /document\.documentElement\.dataset\.style/);
  assert.match(styles, /html\[data-theme="dark"\]/);
  assert.match(styles, /html\[data-style="swiss"\]/);
  assert.match(styles, /html\[data-style="future"\]/);
  assert.match(styles, /html\[data-style="retro"\]/);
  assert.match(styles, /html\[data-style="bauhaus"\]/);
  assert.match(styles, /\.topbar-theme-toggle/);
  assert.doesNotMatch(styles, /\.mobile-theme-toggle/);
  ...
  assert.match(storage, /schemaVersion:\s*16/);
  assert.match(storage, /normalizeDesignStyle/);
  assert.match(storage, /normalizeNavigationOrder/);
  assert.match(storage, /normalizeInspirationCards/);
  assert.match(page, /function VersionHistoryModal/);
  assert.match(page, /function ImportPreviewModal/);
  assert.match(page, /选择并预览备份/);
  assert.match(page, /合并导入/);
  assert.match(page, /覆盖当前工作台/);
  assert.match(page, /APP_VERSION = "1\.5\.0"/);
  assert.match(page, /外观风格/);
  assert.match(page, /安静编辑部/);
  assert.match(page, /瑞士海报/);
  assert.match(page, /未来实验室/);
  assert.match(page, /复古操作台/);
  assert.match(page, /包豪斯积木/);
  assert.match(styles, /\.version-entry/);
  assert.match(styles, /\.inspiration-grid/);
  assert.match(styles, /\.inspiration-card\s*\{[^}]*height:\s*285px/);
  assert.match(styles, /\.inspiration-card-open/);
  assert.match(styles, /\.inspiration-detail-modal/);
  assert.match(page, /className="inspiration-workspace"/);
  assert.match(page, /灵感墙/);
  assert.doesNotMatch(page, /历史灵感墙/);
  assert.match(page, /draggable/);
  assert.match(page, /reorderNavigation/);
  assert.match(page, /nav-drag-handle/);
  assert.match(styles, /\.sidebar nav \.nav-item\.drop-target/);
  assert.match(styles, /\.inspiration-wall/);
  assert.match(styles, /\.content-table/);
...
test("keeps stable distribution instructions release-aware", async () => {
  ...
  assert.match(skill, /更新只替换本地代码/);
  assert.match(skill, /版本记录/);
