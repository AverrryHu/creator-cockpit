...
1. 先向用户说明：更新只替换本地代码，不会将 IndexedDB 数据上传或同步到 GitHub；只要保持相同的浏览器用户、主机名和端口，新版会自动读取并迁移原数据。
2. 让用户从看板左下角打开「版本记录」，点击「导出备份」。在用户确认备份已经成功下载前，不要开始替换版本。
3. 保持原安装目录、浏览器用户、主机名和端口不变。
4. 通过 GitHub Releases API 查询最新稳定标签，并用 `git describe --tags --always` 检查当前安装版本；向用户说明版本变化。
5. 未经明确同意，不要重置或丢弃用户对源代码的本地修改。
6. 如果工作区干净，运行 `git fetch --tags --force`，再切换到选定的稳定标签；如果存在本地修改，先停止并让用户决定保留方式。
7. 运行 `pnpm install --frozen-lockfile`。
8. 运行 `pnpm test`，通过后重新启动看板。
9. 使用更新前相同的本地地址打开看板，确认已有内容、档期、复盘、灵感与个人设置均可正常读取，数据结构迁移已经完成。
10. 如果新版显示空白数据，不要清理浏览器数据或继续写入新的空白工作区。先核对浏览器用户、主机名和端口；仍无法找到时，再让用户通过「设置与备份」选择更新前的 JSON，查看导入摘要后决定合并或覆盖。
